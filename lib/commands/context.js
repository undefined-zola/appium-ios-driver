...
commands.getCurrentContext = async function getCurrentContext () { // eslint-disable-line require-await
  if (this.curContext && this.curContext !== NATIVE_WIN) {
    // debugger;
    console.log(`======================zola: start forin contexts`);
    let contexts = await this.getContextsAndViews(false);
    for (let item of contexts){
      console.log(`======================zola: getCurrentContext:${this.curContext} target id:${item.id}`);
      if (`${WEBVIEW_BASE}${this.curContext}` == item.id) {
        return `${WEBVIEW_BASE}${this.curContext}{--{${item.view.title}--${item.view.url}}}`;
      }
    }
...
commands.getContexts = async function getContexts () {
  ...
  // debugger;
  // let mapFn = context => context.id.toString();  //===zola modify
  let mapFn = context => `${context.id}{--{${context.view.title}--${context.view.url}}}`;
  // debugger;
...
extensions.onPageChange = async function onPageChange (pageChangeNotification) {
  ...
  if (newPages.length) {
    ...
    logger.debug(`We have new pages, selecting page '${newPage}'`);
